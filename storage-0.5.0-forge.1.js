// Keep the mobile preview's saves separate from still-open schema1/2/3/4 tabs.
...
(function (global) {
 ...
 async function selectedOldEntries(name) {
  ...
  try {
   ...
   return await new Promise((resolve,reject)=>{
    ...
    r.onsuccess=()=>{
     const keys=r.result;
     const saves=keys.filter(k=>typeof k==='string' && k.startsWith('/userfs/') && !k.includes('/../') && /\/forge-save-v[1234]\.json$/.test(k));
 ...
 async function migrate(options={}) {
  const oldNames=options.oldNames ?? (options.oldName ? [options.oldName] : ['/userfs-ember-ore-schema4','/userfs-ember-ore-schema3','/userfs-ember-ore-schema2','/userfs']);
  const newName=options.newName||'/userfs-ember-ore-schema5';
  if(!Array.isArray(oldNames) || !oldNames.length || oldNames.some(name=>typeof name!=='string' || !name || name===newName) || typeof newName!=='string') throw new Error('서로 다른 유효한 저장 공간이 필요합니다.');
  ...
  try {
   ...
   const state=await new Promise((resolve,reject)=>{
    ...
    tx.objectStore(FILES).getAllKeys().onsuccess=e=>{hasSave=e.target.result.some(k=>typeof k==='string' && /\/forge-save-v[12345]\.json$/.test(k));};
   ...
   if(!state.hasSave){
    for(const name of oldNames){
     // An unreadable/ambiguous earlier source rejects; only absence of a game save falls through.
     entries=await selectedOldEntries(name);
   ...
   return await new Promise((resolve,reject)=>{
    ...
    marker.onsuccess=()=>{
     ...
     keys.onsuccess=()=>{
      ...
      const hasSave=keys.result.some(k=>typeof k==='string' && /\/forge-save-v[12345]\.json$/.test(k));
      if(hasSave) status='kept_new_progress';
      else for(const [key,value] of entries) files.put(value,key);
      meta.put({version:5,complete:true},'complete');
     };
    };
